...
const showOnlyNegativeChk = document.getElementById("showOnlyNegative");
const highlightNegativesChk = document.getElementById("highlightNegatives");
...
function addCategory(){
    ...
    // Select newly added
    const optionExists = Array.from(txCategorySelect.options).some(opt=>opt.value===c);
    if(optionExists) txCategorySelect.value=c;
}
...
function addTransactionObj(obj){
    const tx={description:String(obj.description||"").trim(), amount:parseFloat(obj.amount)||0, type:obj.type||'expense', frequency:obj.frequency||'irregular', category:obj.category||"", date:toISO(obj.date)||""};
...
function deleteTransaction(index){ if(index<0||index>=transactions.length) return; transactions.splice(index,1); saveTransactions(); renderTransactionTable(); renderProjectionTable(); }
...
function renderTransactionTable(){
    ...
    sorted.forEach((tx,idx)=>{
        ...
        tr.querySelector(".delete-btn").addEventListener("click",()=>{ if(confirm("Delete transaction?")) deleteTransaction(idx); });
    });
}

// ---------------- Projection ----------------
function txOccursOn(tx,dateIso){ if(!tx||!dateIso) return false; if(tx.frequency==='irregular') return tx.date===dateIso; if(!tx.date) return false; const start=new Date(tx.date); const d=new Date(dateIso); if(d<start) return false; if(tx.frequency==='monthly'){ const startDay=start.getDate(); const lastDay=new Date(d.getFullYear(),d.getMonth()+1,0).getDate(); return d.getDate()===Math.min(startDay,lastDay); } if(tx.frequency==='4-weekly'){ const diffDays=Math.floor((d-start)/(1000*60*60*24)); return diffDays>=0 && diffDays%28===0; } return false; }
...
function renderProjectionTable(){
    ...
    for(let d=new Date(start);d<=end;d.setDate(d.getDate()+1)){
        ...
        todays.forEach(t=>{ if(t.type==='income') income+=t.amount; else expense+=t.amount; const catStr=t.category?` (${escapeHtml(t.category)})`:""; descs.push(t.frequency==='irregular'?`<span class="irregular">${escapeHtml(t.description)}${catStr}</span>`:`${escapeHtml(t.description)}${catStr}`); });
        runningBalance += income-expense;
        if(showOnlyNegativeChk.checked && runningBalance>=0) continue;
        const tr=document.createElement("tr"); tr.setAttribute("data-date",iso);
        if(highlightNegativesChk.checked && runningBalance<0) tr.classList.add("negative");
