...
function updateCategoryDropdown() {
    ...
    categories.forEach(cat=>{const opt=document.createElement("option");opt.value=cat;opt.textContent=cat;txCategorySelect.appendChild(opt);});
}
...
function addTransactionObj(obj){
    ...
    if((tx.frequency==='monthly'||tx.frequency==='4-weekly')&&!tx.date){alert("Please choose a start date for recurring transactions."); return;}
...
function renderTransactionTable(){
    ...
    sorted.forEach((tx,sortedIdx)=>{
        ...
        tr.querySelector(".delete-btn").addEventListener("click",(e)=>{
            ...
            if(origIdx===-1){if(confirm("Delete this transaction?")){const mappedIdx=transactions.indexOf(txToDelete);if(mappedIdx>=0)deleteTransaction(mappedIdx);}return;}
...
function renderProjectionTable(){
    ...
    if(!startDate){const tr=document.createElement("tr");tr.innerHTML=`<td colspan="5" class="small">Please set a Start Date in Configuration and press Save to generate the projection.</td>`; projectionTbody.appendChild(tr); return;}
    ...
    for(let d=new Date(start);d<=end;d.setDate(d.getDate()+1)){
        ...
        todays.forEach(t=>{if(t.type==='income')income+=t.amount;else expense+=t.amount; const catStr=t.category?` (${escapeHtml(t.category)})`:""; const label=t.frequency==='irregular'?`<span class="irregular">${escapeHtml(t
